...
const CheckoutPage = async () => {
  const cartCookies = cookies().get('user_id')?.value;
      ...
      const url = await fetch(`http://localhost:3000/api/cart?user_id=${cartCookies}`);
      ...
      console.log(
        ...
        const subtotal = cart.reduce((total:number, item:Cart) => total + item.product_price * item.product_quantity, 0);
          ...
          <div className="space-y-4">
              ...
              >
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 inter">${item.product_price.toFixed(2)}</p>
                  </div>
                </div>
              </div>
            )})}
            ...
            <div className="pt-4 space-y-4 inter">
              <div className="flex justify-between text-sm">
                ...
                <span className="flex gap-2"> <span className="font-medium text-gray-400 line-through gap-2">${subtotal.toFixed(2)}</span><span className="font-medium text-gray-700">${discounted.toFixed(2)}</span></span>
              </div>
              <div className="flex justify-between text-sm">
